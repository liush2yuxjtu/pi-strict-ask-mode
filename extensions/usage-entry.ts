...
import askMode from './ask-mode.ts';
import { createUsageFunnel } from './usage-funnel.ts';

const version = String(JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8')).version);

export default function usageInstrumentedAskMode(pi: ExtensionAPI): void {
  const funnel = createUsageFunnel('pi-strict-ask-mode', version);
